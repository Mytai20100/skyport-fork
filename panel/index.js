...
// Trust proxy so express-rate-limit correctly reads X-Forwarded-For (e.g. behind nginx/docker)
...
app.use(async (req, res, next) => {
  try {
    ...
    // Read theme fresh each request so background/color changes apply immediately
    let currentTheme = theme;
    try {
      const fs = require('node:fs');
      currentTheme = JSON.parse(fs.readFileSync('./storage/theme.json', 'utf8'));
    } catch(e) {}
