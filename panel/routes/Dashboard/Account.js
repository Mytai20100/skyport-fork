...
const avatarUpload = multer({ storage: avatarStorage, limits: { fileSize: 2 * 1024 * 1024 }, fileFilter: (req, file, cb) => { if (file.mimetype.startsWith("image/")) cb(null, true); else cb(new Error("Not an image"), false); } });
...
router.post("/account/avatar", isAuthenticated, avatarUpload.single("avatar"), async (req, res) => {
  try {
